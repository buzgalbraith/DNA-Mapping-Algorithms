...
def get_ref(ref_path):
    ...
    return ref
def bwa(ref, read_dict, num_reads=100, tol=0, num_workers=10):
...
if rank==0:
    ...
    read_dict_data =  [{read:[]} for read in reads]
    
else:
    read_dict_data =  None 
print((read_dict_data))
read_dict_data_chunk = comm.scatter(read_dict_data, root=0)
read_dict_data_chunk = bwa(ref, read_dict, num_reads=num_workers, tol=tol, num_workers=num_workers )
